...
CKPT_DIR = "C:/Lily/voiceKYC/pretrained_models/wespeaker_redimnet2_voxvietnam_voxceleb1_finetuned_v1"
...
MARGIN = 0.2
PATIENCE = 4
FROZEN_MODULES = []  # full unfreeze -- giong cau hinh tot nhat cua ResNet (v6/v7)
MAX_UTTS_PER_SPK_EN = 30  # gioi han so utt/speaker VoxCeleb1 de can bang voi VoxVietnam, tranh EN lan at VI


def main():
    train_dataset = CombinedSpeakerDataset(max_utts_per_spk_en=MAX_UTTS_PER_SPK_EN)
    n_speakers = len(train_dataset.speakers)

    train_loader = DataLoader(
        train_dataset, batch_size=BATCH_SIZE, shuffle=True,
        collate_fn=collate_fn, num_workers=0, drop_last=True,
        # num_workers=0: tren Windows, DataLoader worker dung spawn (khong fork), phai pickle
        # toan bo dataset (55932 item, gom mang audio VoxVietnam trong RAM) sang moi worker
        # process -- gay MemoryError. num_workers=0 chay data loading tren main process, an toan.
    )
    ...
    if os.path.exists(resume_ckpt):
        ...
        print(f"Resume tu checkpoint da luu (epoch {ckpt['epoch']}, Top5={ckpt['result']['top5']*100:.2f}%)")
    else:
    ...
    label = "diem xuat phat (resume)" if start_epoch > 0 else "BASELINE (ReDimNet2-B6-LM, truoc khi fine-tune)"
    print(f"\nDanh gia {label}...")
    ...
    for epoch in range(start_epoch, N_EPOCHS):
        model.train()
        frontend.eval()  # frontend khong co tham so hoc, luon eval
        arc_margin.train()
        t0 = time.time()
