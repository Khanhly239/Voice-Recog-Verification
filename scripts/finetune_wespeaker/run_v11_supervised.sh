...
for i in $(seq 1 "$MAX_RETRY"); do
  echo "=== [supervisor] lan chay $i/$MAX_RETRY : $(date '+%H:%M:%S') ===" \
    | tee -a "$LOG_DIR/v11_supervisor.log"
  python -u finetune_v11.py >> "$LOG_DIR/v11_supervisor.log" 2>&1
  code=$?
  echo "=== [supervisor] ket thuc voi ma $code ===" | tee -a "$LOG_DIR/v11_supervisor.log"

  if [ "$code" -eq 0 ]; then
    echo "[supervisor] train hoan tat binh thuong." | tee -a "$LOG_DIR/v11_supervisor.log"
    break
  fi
  if grep -q "Early stopping" "$LOG_DIR/v11_supervisor.log" 2>/dev/null; then
    echo "[supervisor] da early-stop, khong chay lai." | tee -a "$LOG_DIR/v11_supervisor.log"
    break
  fi
