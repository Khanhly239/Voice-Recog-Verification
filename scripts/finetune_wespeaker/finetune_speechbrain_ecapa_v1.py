# -*- coding: utf-8 -*-
"""Fine-tune SpeechBrain ECAPA-TDNN (speechbrain/spkrec-ecapa-voxceleb) tren VoxVietnam-only,
giong het cong thuc v11 (CosFace m=0.25, MUSAN/RIRS, LR backbone/head tach rieng, valid/test
split -- xem data_split.py) nhung doi kien truc + frontend.

Baseline zero-shot DA DO tren dung benchmark nay (test_cache_full.pkl, xem
baseline_speechbrain_ecapa.py): Top1=84.39% Top5=90.32% EER=7.75% -- gan bang ResNet34
zero-shot (7.86%), KHONG phai 5.34% trong docx cu (so do tu mot mau VoxVietnam khac, cu hon,
khong con luu lai -- xem thao luan trong plan/hoi thoai).

GPU 4GB dung chung: model nay ~20.77M tham so (nang hon nhieu so ResNet34 6.63M, gan bang
ResNet293 28.6M da can batch=4). Dung batch=4 + grad-accum=8 = effective batch 32.
"""
import csv
import os
import sys
...
from data_voxvietnam_speechbrain import VoxVietnamSpeechBrainDataset, collate_fn
from evaluate_voxvietnam_speechbrain import evaluate_eer_voxvietnam
from speechbrain.lobes.models.ECAPA_TDNN import ECAPA_TDNN

CKPT_DIR = "C:/Lily/voiceKYC/pretrained_models/speechbrain_ecapa_v1_voxvietnam_only"
os.makedirs(CKPT_DIR, exist_ok=True)
PRETRAINED_PATH = "C:/Lily/voiceKYC/pretrained_models/spkrec-ecapa-voxceleb/embedding_model.ckpt"

N_EPOCHS = 45
BATCH_SIZE = 4
GRAD_ACCUM = 8  # effective batch 32, giong v8/v10/v11
LR_BACKBONE = 1e-5
LR_HEAD = 1e-3
MARGIN = 0.25
EMBED_DIM = 192  # native cua SpeechBrain ECAPA (KHONG phai 256 nhu ResNet34)
PATIENCE = 10
...
def main():
    train_dataset = VoxVietnamSpeechBrainDataset(musan_rirs=True)
    ...
    print(f"\n=== KET QUA speechbrain_ecapa_v1 (chon bang VALID, bao cao bang TEST) ===")
